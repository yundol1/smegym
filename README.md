...
| `OPENAI_API_KEY` | OpenAI API 키 (선택 - AI 코치 실사용 시 필요) |
...
3. `012_fix_rejected_reupload.sql` - 반려 후 재업로드 수정

### Storage 버킷

다음 3개의 버킷이 필요합니다. 마이그레이션 실행 시 자동 생성됩니다:

- `workout-photos` - 운동 인증 사진 (비공개)
- `profile-photos` - 프로필 사진 (공개)
- `challenge-banners` - 챌린지 배너 (공개)
...
E2E 테스트에는 별도의 테스트 계정 환경 변수가 필요합니다. `.env.test.example`을 참고하여 `.env.test` 파일을 생성하세요:
